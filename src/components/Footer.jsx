...
const Footer = () => {
  return (
    <footer className="bg-[#0a0a0a] border-t border-[#1a1a1a]">
      {/* Top red accent line */}
      <div className="h-0.5 bg-gradient-to-r from-transparent via-[#E50914] to-transparent" />

      <div className="max-w-6xl mx-auto px-6 md:px-10 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-10">
          {/* Branding */}
          <div>
            <span className="text-[#E50914] text-2xl font-black tracking-widest">
              ARBAB
            </span>
            ...
            <ul className="space-y-2.5">
              {NAV_LINKS.map((link) => (
                <li key={link.label}>
                  <a
                    href={link.href}
                    className="text-gray-500 hover:text-white text-sm transition-colors duration-300 hover:translate-x-1 inline-block"
                  >
                    {link.label}
                  </a>
                </li>
            ...
            <div className="flex gap-3">
              {SOCIAL.map((s) => (
                <a
                  ...
                  title={s.name}
                  className="w-9 h-9 rounded-lg bg-[#1a1a1a] border border-[#2F2F2F]
